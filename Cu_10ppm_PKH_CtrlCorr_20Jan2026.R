...
# Load data
df <- read.csv("Cu_10ppm_PKH_CtrlCorr_20Jan2026.csv")
...
p <- ggplot(df_long, aes(x = Strain, y = Value, fill = Fraction)) +
  ...
  labs(
    title = "10 ppm Cu²⁺, Proteinase K, cells boiled, Ctrl-corr.",
    x = NULL,
    y = "Cu²⁺ (ppm)"
  ) +
...
ggsave("Cu_10ppm_PKH_CtrlCorr_20Jan2026.png", p, width = 10, height = 6, dpi = 300)
